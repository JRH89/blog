...
export async function PUT(req, ctx) {
	...
	try {
		...
		if (blog?.authorId?._id.toString() !== decodedToken._id.toString()) {
			return new Response(JSON.stringify({msg: 'Only the author can update this blog'}), {status: 403})
		}
...
export async function DELETE(req, ctx) {
	...
	try {
		const blog = await Blog.findById(id).populate('authorId')
		if (blog?.authorId?._id.toString() !== decodedToken._id.toString()) {
			return new Response(JSON.stringify({msg: 'Only the author can delete this blog'}), {status: 403})
		}
